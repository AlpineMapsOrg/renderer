...
highp vec3 TurboColormap(highp float x) {
  ...
  const highp vec2 kRedVec2 = vec2(-152.94239396, 59.28637943);
  const highp vec2 kGreenVec2 = vec2(4.27729857, 2.82956604);
  const highp vec2 kBlueVec2 = vec2(-89.90310912, 27.34824973);

  x = clamp(x, 0., 1.);
  highp vec4 v4 = vec4( 1.0, x, x * x, x * x * x);
  highp vec2 v2 = v4.zw * v4.z;
  return vec3(
