...
TEST_CASE("nucleus/tile_scheduler/slot limiter")
{
    ...
    SECTION("receiving tiles frees up slots")
    {
        ...
        sl.deliver_quad(tile_types::DataQuad { tile::Id { 0, { 0, 0 } } });
        CHECK(sl.slots_taken() == 1);
        sl.deliver_quad(tile_types::DataQuad { tile::Id { 1, { 0, 0 } } });
    ...
    SECTION("receiving tiles triggers processing of queue")
    {
        ...
        sl.deliver_quad(tile_types::DataQuad { tile::Id { 1, { 0, 1 } } });
        CHECK(sl.slots_taken() == 1);
        sl.deliver_quad(tile_types::DataQuad { tile::Id { 2, { 0, 0 } } });
        CHECK(sl.slots_taken() == 0);
        CHECK(spy.size() == 4);
    ...
    SECTION("receiving tiles triggers processing of queue")
    {
        ...
        for (unsigned i = 0; i < 5; ++i) {
            sl.deliver_quad(tile_types::DataQuad { tile::Id { i, { 0, 0 } } });
            CHECK(sl.slots_taken() == 2);
        ...
        // running out of tile requests
        sl.deliver_quad(tile_types::DataQuad { tile::Id { 5, { 0, 0 } } });
        CHECK(sl.slots_taken() == 1);
        sl.deliver_quad(tile_types::DataQuad { tile::Id { 6, { 0, 0 } } });
        CHECK(sl.slots_taken() == 0);
        CHECK(spy.size() == 7);
    ...
    SECTION("updating request list omits requesting in flight tiles again")
    {
        ...
        sl.deliver_quad(tile_types::DataQuad { tile::Id { 3, { 1, 0 } } });
        CHECK(sl.slots_taken() == 1);

        sl.deliver_quad(tile_types::DataQuad { tile::Id { 2, { 1, 0 } } });
        CHECK(sl.slots_taken() == 0);
    }
    ...
    SECTION("delivered quads are sent on")
    {
        ...
        sl.deliver_quad(tile_types::DataQuad { tile::Id { 0, { 0, 0 } }, 4, {} });
        REQUIRE(spy.size() == 1);
        CHECK(spy[0][0].value<tile_types::DataQuad>().id == tile::Id { 0, { 0, 0 } });

        sl.deliver_quad(tile_types::DataQuad { tile::Id { 1, { 2, 3 } }, 4, {} });
        REQUIRE(spy.size() == 2);
        CHECK(spy[1][0].value<tile_types::DataQuad>().id == tile::Id { 1, { 2, 3 } });
    }
}
