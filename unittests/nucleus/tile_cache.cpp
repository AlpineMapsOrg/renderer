...
TEST_CASE("nucleus/tile/cache")
{
    ...
    SECTION("insert and visit")
    {
        ...
        std::unordered_set<Id, Id::Hasher> visited;
    ...
    SECTION("purge: all elements equal, large zoom levels first")
    {
        ...
        CHECK(std::find_if(purged.cbegin(), purged.cend(), [](const auto& v) { return v.id == Id { 2, { 0, 0 } }; }) != purged.cend());
        CHECK(std::find_if(purged.cbegin(), purged.cend(), [](const auto& v) { return v.id == Id { 6, { 4, 3 } }; }) != purged.cend());
        CHECK(std::find_if(purged.cbegin(), purged.cend(), [](const auto& v) { return v.id == Id { 3, { 0, 0 } }; }) != purged.cend());
    ...
    SECTION("purge: elements coming in earlier are purged first")
    {
        ...
        CHECK(std::find_if(purged.cbegin(), purged.cend(), [](const auto& t) { return t.id == Id { 0, { 0, 0 } }; }) != purged.end());
        CHECK(std::find_if(purged.cbegin(), purged.cend(), [](const auto& t) { return t.id == Id { 1, { 0, 1 } }; }) != purged.end());
        CHECK(cache.n_cached_objects() == 2);
        CHECK(cache.contains({ 1, { 0, 0 } }));
    ...
    SECTION("purge: visit updates the time")
    {
        ...
        const std::unordered_set<Id, Id::Hasher> newer_tiles = { { 1, { 0, 0 } }, { 1, { 0, 1 } }, { 1, { 1, 0 } } };
        QThread::msleep(2);
    ...
    const auto create_test_tile = [](const Id& id, int meta_data = 0) {
    ...
    const auto verify_tile = [](const auto& cache, const Id& id, int meta_data = 0) {
    ...
    SECTION("reading disk cache back fails on bad version") {
        ...
        {
            Cache<DiskWriteTestTile2> cache;
            CHECK(!cache.read_from_disk(path).has_value());
        }
        std::filesystem::remove_all(path);
    }
