...
TEST_CASE("nucleus/tile/rate limiter")
{
    ...
    SECTION("tile requests are forwarded")
    {
        ...
        rl.request_quad(Id { 0, { 0, 0 } });
        REQUIRE(spy.size() == 1);
        CHECK(spy[0][0].value<Id>() == Id { 0, { 0, 0 } });
        rl.request_quad(Id { 1, { 0, 0 } });
        REQUIRE(spy.size() == 2);
    ...
    SECTION("slots are freed up after some time and request queue is processed")
    {
        ...
        rl.request_quad(Id { 1, { 0, 0 } });
        rl.request_quad(Id { 2, { 0, 0 } });
        rl.request_quad(Id { 3, { 0, 0 } });
        rl.request_quad(Id { 4, { 0, 0 } });
        rl.request_quad(Id { 5, { 0, 0 } });
        REQUIRE(spy.size() == 2);
        ...
        for (int i = 0; i < spy.size(); ++i)
            CHECK(spy[i][0].value<Id>() == Id { unsigned(i), { 0, 0 } });
    }
    ...
    SECTION("request queue is handled correctly, when requests come in one after the other")
    {
        ...
        }
        {
            ...
            unittests::RateTester tester(&rl);
            QSignalSpy spy(&rl, &RateLimiter::quad_requested);
            rl.request_quad(Id { 0, { 0, 0 } }); // sent at t=0
            rl.request_quad(Id { 1, { 0, 0 } }); // sent at t=0
            rl.request_quad(Id { 2, { 0, 0 } }); // sent at t=11
            CHECK(spy.size() == 2);
            test_helpers::process_events_for(12 * timing_multiplicator);
            CHECK(spy.size() == 3);
            rl.request_quad(Id { 3, { 0, 0 } }); // sent at t=12
            CHECK(spy.size() == 4);
            rl.request_quad(Id { 4, { 0, 0 } }); // sent at t=22
            rl.request_quad(Id { 5, { 0, 0 } }); // sent at t=23
            CHECK(spy.size() == 4);
            test_helpers::process_events_for(12 * timing_multiplicator);
            rl.request_quad(Id { 6, { 0, 0 } });
            rl.request_quad(Id { 7, { 0, 0 } });
            rl.request_quad(Id { 8, { 0, 0 } });
            CHECK(spy.size() == 6);
        ...
        }
        {
            ...
            const auto make_request = [&rl, &request_no]() { rl.request_quad(Id { request_no++, { 0, 0 } }); };
            QSignalSpy spy(&rl, &RateLimiter::quad_requested);
            make_request();
            CHECK(spy.size() == 1);
