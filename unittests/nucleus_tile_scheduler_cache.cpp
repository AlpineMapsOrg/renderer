...
TEST_CASE("nucleus/tile_scheduler/cache")
{
    ...
    SECTION("purge: visited elements are purged later than others") // todo: expand these tests. currently we are probably not catching everything. // erm, not we are not. orange is currently touched and stays in the cache.
    {
        nucleus::tile_scheduler::Cache<TestTile> cache;
        cache.insert({
            TestTile { { 0, { 0, 0 } }, "green" },
