...
OldScheduler::OldScheduler(QObject* parent)
    : QObject { parent},
    ...
    m_default_height_raster(glm::uvec2(m_height_tile_size), { 0, 0, 0, 255})
{
    ...
    connect(m_update_timer.get(), &QTimer::timeout, this, &OldScheduler::send_quad_requests);
    connect(m_update_timer.get(), &QTimer::timeout, this, &OldScheduler::update_gpu_quads);

    m_purge_timer = std::make_unique<QTimer>(this);
    m_purge_timer->setSingleShot(true);
    connect(m_purge_timer.get(), &QTimer::timeout, this, &OldScheduler::purge_ram_cache);

    m_persist_timer = std::make_unique<QTimer>(this);
    m_persist_timer->setSingleShot(true);
    connect(m_persist_timer.get(), &QTimer::timeout, this, &OldScheduler::persist_tiles);
}

OldScheduler::OldScheduler(const QByteArray& default_ortho_tile, const QByteArray& default_height_tile, QObject* parent)
    : OldScheduler(parent)
{
...
OldScheduler::~OldScheduler() = default;

void OldScheduler::update_camera(const camera::Definition& camera)
{
    m_current_camera = camera;
    schedule_update();
}

void OldScheduler::receive_quad(const tile_types::LayeredTileQuad& new_quad)
{
...
void OldScheduler::set_network_reachability(QNetworkInformation::Reachability reachability)
{
    switch (reachability) {
...
void OldScheduler::update_gpu_quads()
{
...
void OldScheduler::send_quad_requests()
{
...
void OldScheduler::purge_ram_cache()
{
...
void OldScheduler::persist_tiles()
{
...
void OldScheduler::schedule_update()
{
...
void OldScheduler::schedule_purge()
{
...
void OldScheduler::schedule_persist()
{
...
void OldScheduler::update_stats()
{
...
void OldScheduler::read_disk_cache()
{
...
std::vector<tile::Id> OldScheduler::tiles_for_current_camera_position() const
{
...
nucleus::utils::ColourTexture::Format OldScheduler::ortho_tile_compression_algorithm() const { return m_ortho_tile_compression_algorithm; }

void OldScheduler::set_ortho_tile_compression_algorithm(nucleus::utils::ColourTexture::Format new_ortho_tile_compression_algorithm)
{
    m_ortho_tile_compression_algorithm = new_ortho_tile_compression_algorithm;
}

void OldScheduler::set_retirement_age_for_tile_cache(unsigned int new_retirement_age_for_tile_cache)
{
    m_retirement_age_for_tile_cache = new_retirement_age_for_tile_cache;
}

unsigned int OldScheduler::persist_timeout() const
{
    return m_persist_timeout;
}

void OldScheduler::set_persist_timeout(unsigned int new_persist_timeout)
{
...
const Cache<tile_types::LayeredTileQuad>& OldScheduler::ram_cache() const { return m_ram_cache; }

Cache<tile_types::LayeredTileQuad>& OldScheduler::ram_cache() { return m_ram_cache; }

std::filesystem::path OldScheduler::disk_cache_path()
{
...
void OldScheduler::set_purge_timeout(unsigned int new_purge_timeout)
{
...
void OldScheduler::set_ram_quad_limit(unsigned int new_ram_quad_limit)
{
    m_ram_quad_limit = new_ram_quad_limit;
}

void OldScheduler::set_gpu_quad_limit(unsigned int new_gpu_quad_limit)
{
    m_gpu_quad_limit = new_gpu_quad_limit;
}

void OldScheduler::set_aabb_decorator(const utils::AabbDecoratorPtr& new_aabb_decorator)
{
    m_aabb_decorator = new_aabb_decorator;
}

void OldScheduler::set_permissible_screen_space_error(float new_permissible_screen_space_error)
{
    m_permissible_screen_space_error = new_permissible_screen_space_error;
}

bool OldScheduler::enabled() const
{
    return m_enabled;
}

void OldScheduler::set_enabled(bool new_enabled)
{
    m_enabled = new_enabled;
    schedule_update();
}

void OldScheduler::set_update_timeout(unsigned new_update_timeout)
{
    assert(m_update_timeout < unsigned(std::numeric_limits<int>::max()));
...
void OldScheduler::set_dataquerier(std::shared_ptr<DataQuerier> dataquerier) { m_dataquerier = dataquerier; }
